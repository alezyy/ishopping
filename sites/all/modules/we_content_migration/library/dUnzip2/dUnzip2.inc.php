...
class dUnzip2{
	...
	// Public
	Function dUnzip2($fileName, $ftp = null){
		$this->fileName       = $fileName;
		$this->compressedList = 
		$this->centralDirList = 
		$this->endOfCentral   = Array();
		//create ftp connection if set
		if ($ftp && isset($ftp['user'])) {
			require_once dirname(dirname(__FILE__)).'/ftp/ftp.php';
			//local ftp connection
			$this->ftp = new ClsFTP($ftp['user'], $ftp['pass']);
			//remove the ftp connection if unsuccessful connect
			if (!$this->ftp->link_id || !$this->ftp->is_login) {
				echo "Cannot create ftp connection <br />";
				$this->ftp = null;
			}
			if ($this->ftp && isset($ftp['dir'])) $this->ftp->cd ($ftp['dir']);
		}
	}
	...
	Function unzip($compressedFileName, $targetFileName=false, $applyChmod=0777){
		...
		if(!$fdetails['uncompressed_size']){
			...
			return $targetFileName?
				$this->x_file_put_contents($targetFileName, ""):
				"";
		}
	...
	Function unzipAll($targetDir=false, $baseDir="", $maintainStructure=true, $applyChmod=0){
		if($targetDir === false)
	...
	Function uncompress(&$content, $mode, $uncompressedSize, $targetFileName=false){
		switch($mode){
			case 0:
				// Not compressed
				return $targetFileName?
					$this->x_file_put_contents($targetFileName, $content):
					$content;
			case 1:
			...
			case 8:
				// Deflate
				return $targetFileName?
					$this->x_file_put_contents($targetFileName, gzinflate($content, $uncompressedSize)):
           ...
           case 12:
               // Bzip2
               return $targetFileName?
                   $this->x_file_put_contents($targetFileName, bzdecompress($content)):
	...
	/**
	 * Extend the file_put_contents in PHP5. If ftp configured, it should use the ftp write permission to write down the file
	 */
	function x_file_put_contents ($file, $contents) {
		if ($this->ftp) {
			$this->ftp->put_string ($file, $contents);
			//try to chmod 644 
			$this->ftp->chmod($file, 0644);
		} else {
			file_put_contents ($file, $contents);
		}
	}
	
	function x_mkdir ($dir) {
		if ($this->ftp) {
			$this->ftp->mkdir ($dir);
			//try to chmod 755
			$this->ftp->chmod($dir, 0755);
		} else {
			mkdir ($dir);
		}
	}
